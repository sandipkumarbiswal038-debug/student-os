...
const attendanceSessionId = (record) => record.class_session?.id ?? record.class_session_id ?? record.class_session?.pk ?? record.class_session;
const assignedFaculty = (session) => session.faculty?.id ?? session.faculty_id ?? session.faculty?.college_email ?? session.faculty_email ?? session.faculty?.email ?? session.faculty;

const belongsToFaculty = (session, faculty) => {
  const assigned = assignedFaculty(session);
  const identities = [faculty?.id, faculty?.college_email, faculty?.email, faculty?.name];
  return assigned != null && identities.some((identity) => matches(assigned, identity));
};

function AttendanceHistory({ classSessionId }) {
  ...
  const load = async () => {
    ...
    try {
      const [records, students, sessions] = await Promise.all([
        ...
        studentApi.list(),
        authenticatedRequest("/api/class-sessions/", localStorage.getItem("authToken")).catch(() => []),
      ]);
      const byId = new Map(students.map((student) => [String(student.id), student]));
      const savedUser = JSON.parse(localStorage.getItem("currentUser") || "null");
      const facultySessionIds = new Set(
        asList(sessions)
          .filter((session) => belongsToFaculty(session, savedUser))
          .map((session) => String(session.id))
      );
      // Never render the unfiltered attendance endpoint: it may include
      // records created by another faculty member.
      const sessionRecords = asList(records).filter((record) => {
        const recordSessionId = String(attendanceSessionId(record));
  ...
  const presentCount = history.filter((item) => String(item.status).toLowerCase() === "present").length;
  const absentCount = history.filter((item) => String(item.status).toLowerCase() === "absent").length;
